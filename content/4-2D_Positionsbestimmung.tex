...
\begin{figure}
	...
	\caption[Verknüpfung der Scantools]{Verknüpfung von \glqq laser\_ortho\_projector \grqq - und \glqq laser\_scan\_matcher\grqq -Knoten} %
...
Entfernungsdaten eines Laserumlaufs bestehen aus mehreren diskreten Abtastungen (Abbildung~\ref{fig:l-frame}). Übergeben werden sie in Form von Entfernungen $r_i$ in einem Array. Mittels der Schrittweite von $0.25^\circ$ lassen sich anhand des Indizes $i$ für jeden Messpunkt ein Winkel $\gamma_i$ zuweisen. 
\begin{equation}
...
Da der Bezugspunkt des o-frames im Schwerpunkt des Quadrocoptes liegen soll, in dem auch der b-frame seinen Ursprung hat, ist es erforderlich die Laserdaten vom l-frame ins b-frame zu transformieren. Wie schon in Kapitel \ref{sec:koordinatensysteme&transformationen} erwähnt, handelt es sich um eine konstante Transformation. Genauer gesagt, um einen Offset von $10~cm$ auf der $z^b$-Achse, denn der Laser ist oberhalb des Quadrocopterschwerpunktes montiert.
...
Nachdem die Laserpunkte im b-frame definiert sind, kann die Transformation der Umgebungsdaten in die xy-Ebene des o-frames erfolgen. Angesichts der Tatsache, dass die Winkel $\phi$ und $\theta$ als Verdrehung um die Achsen des o-frames definiert sind, erfordert es zur Umrechnung der Laserdaten die in Kapitel \ref{sec:koordinatensysteme&transformationen} eingeführte Gleichung (\ref{eq:inverse_transM}) zur inversen Koordinatentransformation. Dabei wird der Yaw-Winkel zu Null gesetzt ($ \psi = 0 $). Grund hierfür ist, dass die Ausrichtung der Flugrichtung in der zweidimensionalen Ebene mit der des b-frames übereinstimmen sollen. Daraus ergibt sich für die Transformationsmatrix,   
...
Die Beschreibung $S$ der Laserscan im o-frame ist die Basis für die im anschließende Kapitel \ref{sec:laser_scan_matching} behandelte Positionsbestimmung in der zweidimensionalen Ebene des n-frames. 


\section[Positonsbestimmung über Scanmatching]{Positionsbestimmung über scanmatching}
...
\begin{figure}
	\centering{
		...
		\hspace{0.1\textwidth}
		\subfloat[ $ P_{neu} $ über matching von $S_{neu}$ auf $S_{ref}$]{
...
Über $\tilde{T}$ ist die Abweichung der Schwerpunkte translatorisch als auch rotatorisch beschrieben. Damit beide Schwerpunkte genau übereinander liegen ist $\tilde{T} = 0$ zusetzen. Daraus ergibt sich:
...
Und damit eine Fehlergleichung, die ausschließlich von der Rotation abhängig ist und dessen Betrag sich wie folgt darstellt: 
...
Aufgrund der Addition trigonometrischer Funktionen besitzt der Summand ein Maximum für $ \Delta\psi^o $, wenn
...
Für die Positionsbestimmung im n-frame wird mit der ersten Position $ P_{ref} $ der Bezugspunkt des Navigationskoordinatensystems festgelegt. Darauf aufbauend werden die weiteren Translationen bzw. Rotationen addiert. 
