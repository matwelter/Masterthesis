...
Ebenfalls lässt sich die Geschwindigkeit aufgrund des Weg-Zeit-Gesetzes (\ref{eq:weg-zeit}) über Ableitung der Quadrocopterpositionen bestimmen. Möglich ist dies, da der Laser die Umgebung mit einer festen Abtastzeit $ T_l = 25ms $ ($ 40Hz) $) die Umgebung scannt. So kann über die mittels  Scanmatchingverfahren ermittelte Positionen die Geschwindigkeit durch eine diskrete Ableitung errechnet werden.
\begin{equation}
v^n_k = \frac{P^n_k-P^n_{k-1}}{T_l}
\end{equation}
Dabei ist das Positionssignal aufgrund des Sensorrauschen des Lasers, der für die Projektion benötigten Gyrosskope sowie der Varianz des icp Schätzalgorithmuses verrauscht. Dieses Positonsrauschen $ r_P $ überträgt sich auch auf die Geschwindigkeit. 
\begin{equation}
v^n_k = \frac{P^n_k-P^n_{k-1}}{T_l} + \frac{r_{P_k} -r_{P_{k-1}}}{T_l}
\end{equation}
 Diese führt jedoch, nicht dazu das wie bei der Geschwindigkeitsbestimmung mittels der Beschleunigungssensoren, das der schätzwert und realwert weg divergiert. Nicht desto trotz ist das Rauschen unerwünscht,da es auf den differentiellen Teil des Folgereglers einwirkt. Es geht somit darum den Rauschanteil zu minimieren. Möglich ist dies zwar anhand eines digitalen Filter. Allerdings führen das IIR als auch das FIR Filter zu einer Phasenverschiebung \cite{Thede04}. Damit entspricht der ausgegebene Geschwindigkeit der des  Positionwertes zum Zeitpunk $ t =k \cdot T $. Dies führt zu einer Destabilisierung des Reglers. Der Einfluss ist umso größer je dynamischer der Flug des Quadrocopters ist. So ist eine Möglichkeit gesucht, die eine bestmögliche Filterung  aufweist und eine möglichst geringen Phasenverschiebung aufweist.


%Beobachter dynamik Analog dem Folgeregler über die Polvorgabe vorgegeben werden
