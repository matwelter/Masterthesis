...
\label{sec:geschwReg}
In der ursprünglichen Ausschreibung zu dieser Masterthesis ist die Implementierung einer Geschwindigkeitsregelung als Schwerpunkt der Arbeit ausgegeben. \DIFdelbegin \DIFdel{Deshalb wird abschließenden }\DIFdelend \DIFaddbegin \DIFadd{In Folge dessen wird }\DIFaddend in diesem Kapitel \DIFaddbegin \DIFadd{abschließend  }\DIFaddend die Anwendbarkeit der Positionsregelung als Geschwindigkeitsregelung mittels eines Testergebnisses offengelegt.  

Die Vorgabe der Geschwindigkeiten erfolgt im o-frame, \DIFdelbegin \DIFdel{das }\DIFdelend \DIFaddbegin \DIFadd{dass }\DIFaddend heißt es wird bestimmt mit welcher Geschwindigkeit sich der Quadrocopter vorwärts, rückwärts oder seitlich fortzubewegen hat. \DIFdelbegin \DIFdel{Da die Positionsregelung allerdings }\DIFdelend \DIFaddbegin \DIFadd{Nachdem die Positionsregelung gleichwohl }\DIFaddend auf den Koordinaten des \DIFdelbegin \DIFdel{Navigationskoordinatensystem }\DIFdelend \DIFaddbegin \DIFadd{Navigationskoordinatensystems }\DIFaddend aufbaut, ist die gewünschte Geschwindigkeit zunächst ins n-frame zu transformieren. Dort sind diese\DIFaddbegin \DIFadd{, }\DIFaddend wie im Kapitel \ref{sec:referenzmodell} beschrieben\DIFdelbegin \DIFdel{aufzuintegrieren. So }\DIFdelend \DIFaddbegin \DIFadd{, zu integrieren. Auf diese Weise }\DIFaddend erfolgt für Geschwindigkeiten ungleich \DIFdelbegin \DIFdel{null eine stetig }\DIFdelend \DIFaddbegin \DIFadd{Null eine stetige }\DIFaddend Verschiebung der anzufliegenden Sollposition. Die Größe der Positionsveschiebung ist dabei \DIFdelbegin \DIFdel{Proportional }\DIFdelend \DIFaddbegin \DIFadd{proportional }\DIFaddend zu den geforderten Geschwindigkeiten. Da der Quadrocopter mittels der Positionsregelung der Sollposition folgt (Abbildung \ref{fig:velpos}), spricht man auch von einer \DIFdelbegin \DIFdel{pseudo Geschwindigkeitsregelung}\DIFdelend \DIFaddbegin \DIFadd{Pseudo-Geschwindigkeitsregelung}\DIFaddend .
 \begin{figure}
...
Abbildung \ref{fig:velof} beleuchtet den zur \DIFdelbegin \DIFdel{Positionsschiebung }\DIFdelend \DIFaddbegin \DIFadd{Positionsverschiebung }\DIFaddend führenden Geschwindigkeitsverlauf im o-frame. Die Vorgabe der Geschwindigkeit erfolgte dabei über den Bedienhebel der Fernsteuerung, der bei manueller Steuerung zur Vorgabe der Neigungswinkel dient. Bei aktiver Geschwindigkeitsregelung werden die Rohdaten dieses Bedienhebel jedoch Geschwindigkeitsvorgaben im o-frame zugeordnet. Der Umrechnungsfaktor kann \DIFdelbegin \DIFdel{dabei }\DIFdelend beliebig festgelegt werden, solange der Quadrocopter der maximal einforderbaren Geschwindigkeit folgen kann. \DIFdelbegin \DIFdel{So Betrug }\DIFdelend \DIFaddbegin \DIFadd{In diesem Fall betrug }\DIFaddend der Faktor bei der Durchführung der Messung $ 0.0004 $, das \DIFdelbegin \DIFdel{bedeutet ein }\DIFdelend \DIFaddbegin \DIFadd{entspricht einem }\DIFaddend Rohwert von $ 1000 $ \DIFdelbegin \DIFdel{entspricht }\DIFdelend \DIFaddbegin \DIFadd{und }\DIFaddend einer geforderten Geschwindigkeit von $ 0.4 \frac{m}{s} $.

Um \DIFdelbegin \DIFdel{überprüfen zu können}\DIFdelend \DIFaddbegin \DIFadd{zu prüfen}\DIFaddend , ob sich für den Stellwertverlauf über die Fernbedienung (Abbildung \ref{fig:vel_nickheb} und Abbildung \ref{fig:vel_nickheb}) die geforderten Geschwindigkeiten aus der Sicht des Quadrocopters einstellen, sind die vom Referenzmodell generierte Sollgeschwindigkeit, als auch die vom Beobachter geschätzte Geschwindigkeit ins o-frame transformiert worden (Abbildung \ref{fig:vel_vxof} und Abbildung \ref{fig:vel_vyof}). Auf den ersten Blick fällt auf, \DIFdelbegin \DIFdel{das }\DIFdelend \DIFaddbegin \DIFadd{dass }\DIFaddend der qualitative Verlauf der Vorgaben mit der sich tatsächlich einstellenden Geschwindigkeit übereinstimmt. \DIFdelbegin \DIFdel{Zu sehen ist jedoch auch, das }\DIFdelend \DIFaddbegin \DIFadd{Weiterhin ist zu beobachten, dass }\DIFaddend sich Geschwindigkeitsvorgaben des Bedienhebels gedämpft und \DIFdelbegin \DIFdel{Phasenverschoben }\DIFdelend \DIFaddbegin \DIFadd{phasenverschoben }\DIFaddend auf den Sollwertverlauf des Referenzmodells auswirken, \DIFdelbegin \DIFdel{deutlicher sieht man dies auch }\DIFdelend \DIFaddbegin \DIFadd{deutlich erkennbar }\DIFaddend in Abbildung \ref{fig:velpos}. Zu erklären ist \DIFdelbegin \DIFdel{dies }\DIFdelend \DIFaddbegin \DIFadd{das }\DIFaddend durch die Zeitkonstanten des Referenzmodells, die für \DIFdelbegin \DIFdel{diese }\DIFdelend \DIFaddbegin \DIFadd{diesen }\DIFaddend Flugversuch über die Eigenfrequenz $ \omega_0 = 1.5 $ und die Dämpfung $ D = 1 $ festgelegt sind. Ist \DIFdelbegin \DIFdel{gewollt, das }\DIFdelend \DIFaddbegin \DIFadd{gewünscht, dass }\DIFaddend sich Vorgaben über \DIFdelbegin \DIFdel{den }\DIFdelend die Fernsteuerung schneller einstellen, ist \DIFdelbegin \DIFdel{deshalb }\DIFdelend \DIFaddbegin \DIFadd{daher }\DIFaddend die Eigenfrequenz $ \omega_0$ des Referenzmodells zu erhöhen. Dies wird auch empfohlen\DIFaddbegin \DIFadd{, }\DIFaddend um ein besseres Feedback über geforderte Geschwindigkeiten zu erlangen.  

Abschließend bewertet\DIFdelbegin \DIFdel{zeigt }\DIFdelend \DIFaddbegin \DIFadd{, zeigen }\DIFaddend die Messdaten dieses Flugversuches (Abbildung \ref{fig:velof}), dass \DIFdelbegin \DIFdel{die sich }\DIFdelend \DIFaddbegin \DIFadd{sich die }\DIFaddend auf dem \gls{hlp} befindliche Positionsregelung auch \DIFdelbegin \DIFdel{den bei Ausschreibung formulierten Wunsch }\DIFdelend \DIFaddbegin \DIFadd{dem formulierten Wunsch der Ausschreibung  }\DIFaddend nach einer Geschwindigkeitsregelung erfüllt. Womit die Arbeit ihren Abschluss findet.
